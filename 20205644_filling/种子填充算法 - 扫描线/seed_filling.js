...
function filling_start() {//进行填充
   ...
   // console.log(zuobiao);
    ...
    while(mystack.length!=0){//扫描线
        [tmp_x, tmp_y] = mystack.pop();
        tmp_xl = tmp_x;
        tmp_xr = tmp_x;
       // console.log('x',tmp_x,'y',tmp_y);
        //console.log('栈的长度',mystack.length);
        while(if_colored(tmp_xr+1,tmp_y)==false){//说明没有着色
            draw_dot(tmp_xr + 1, tmp_y);
            tmp_xr += 1;
            Xr = tmp_xr;
            XR = tmp_xr;
            //console.log('栈的长度',mystack.length);
        }
        while(if_colored(tmp_xl,tmp_y)==false){//说明没有着色
            draw_dot(tmp_xl , tmp_y);
            tmp_xl -= 1;
            Xl = tmp_xl;
            XL = tmp_xl;
           /// console.log('栈的长度',mystack.length);
        ...
        while (XR >= XL) {
            if (if_colored(XR, tmp_y + 1) == false) {
                mystack.push([XR, tmp_y + 1]);
                break;
            }
            XR--;
            //console.log("下方",XR,XL);
        }
        while (Xr >= Xl) {
            if (if_colored(Xr, tmp_y - 1) == false) {
                mystack.push([Xr, tmp_y - 1]);
